...
fig.suptitle(r'Left: maximum amplitude charge current. Right: derivative $dI(\omega)/d\omega$')
...
# Labels
ax.set_xlabel(r'Frequency $\omega$ [rad/ns]')
ax.set_ylabel(r'Maxiumum amplitude [a.u.]')
...
frequencies_cut = frequencies[:-1]

# X axis range
plt.xlim(10, 300)

# Plotting
ax.plot(frequencies_cut, DIDHx, label = r'$I_x$')
ax.plot(frequencies_cut, DIDHy, label = r'$I_y$')
ax.plot(frequencies_cut, DIDHz, label = r'$I_z$')
...
# Labels
ax.set_xlabel(r'Frequency $\omega$ [rad/ns]')
ax.set_ylabel(r'FMR signal $dI(\omega)/d\omega$ [a.u.]')
ax.tick_params(labelleft=False)    

# Saving
plt.savefig('figuren/amplitudeplot_and_DIDH.pdf' , bbox_inches = 'tight')
