...
deriv = np.diff(solution)
cross = np.cross(solution,deriv)
sigma = [1,0,0]
crosss = np.cross(cross,sigma)
...
ax.plot(t_span, crosss[:, 0], label = r'$M_x$')
ax.plot(t_span, crosss[:, 1], label = r'$M_y$')
ax.plot(t_span, crosss[:, 2], label = r'$M_z$')
